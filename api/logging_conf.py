...
from api.config import ProdConfig, DevConfig, config


def obfuscated(email: str, obfuscate_length: int) -> str:
    characters = email[:obfuscate_length]
    first, last = email.split("@")
    return characters + ("*" * (len(first) - obfuscate_length)) + "@" + last
    ...
    dictConfig(
        {
            ...
            "formatters": {
                ...
                "file": {
                    "class": "pythonjsonlogger.jsonlogger.JsonFormatter", # logging.Formatter
                    "datefmt": "%Y-%m-%dT%H:%M:%S",
